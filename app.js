...
function renderAnalysis() {
  ...
  document.querySelector('#typeDashboard').innerHTML = types.length ? types.map((item) => `<button class="type-dashboard-card" data-type-filter="${escapeHtml(item.type)}" type="button"><div class="type-dashboard-head"><strong>${escapeHtml(item.type)}</strong><b>${item.accuracy.toFixed(1)}%</b></div><div class="type-dashboard-track"><i style="width:${item.accuracy}%"></i></div><div class="type-dashboard-metrics"><span><b>${item.total}</b>题</span><span><b>${item.correct}</b>对</span><span><b>${item.total - item.correct}</b>错</span><span><b>${item.minutes}</b>分钟</span><span><b>${item.speed.toFixed(2)}</b>分/题</span></div><small class="type-dashboard-errors">${item.errorTypes || '暂未填写错误类型'}</small></button>`).join('') : '<span class="required-note">填写记录后显示题型详细数据</span>';
